...
stg_eval = read.table("~/git/misc-analyses/erc/data/erc_2020_StG_evaluated_projects_all_panels.tab",header=TRUE,sep="\t",row.names=1)
stg_grant = read.table("~/git/misc-analyses/erc/data/erc_2020_StG_granted_projects_all_panels.tab",header=TRUE,sep="\t",row.names=1)
stg_eval
stg_eval
stg_grant
...
################################################################################
### OVERVIEW BARPLOT of all applications
###

bdm = as.matrix(rbind( stg_grant_sums, stg_eval_sums - stg_grant_sums ))
pdf(file="~/git/misc-analyses/erc/erc_2020_StG_all_panels_overview_barplot.pdf", width=7, height=6)
par(mar=c(3,4.5,4,1))
barmain = "ERC Starting Grants 2007-2020"
b=barplot( bdm, names=rev(sub("X","",names(stg_eval))), ylim=c(0,10000), 
           col=c("#007837","#eed4a2ff"), cex.names=0.8, cex.axis=1.4, main=barmain  )
legend(6,9000,legend=c("Submitted", "Granted"), fill=c("#eed4a2ff", "#007837"), bty='n', cex=1.5)
text(b, stg_eval_sums + max(bdm)*0.05, stg_eval_sums )
text(b, stg_grant_sums + max(bdm)*0.05, round(stg_grant_sums/stg_eval_sums,2) )
dev.off()

###
### plot each country as a barplot
###
country_names_mod = gsub("Bosnia and Herzegovina","Bosnia Herzegovina",gsub("The Former Yugoslav Republic Of Macedonia","Macedonia",row.names(stg_eval) ) )
pdf(file="~/git/misc-analyses/erc/erc_2020_StG_barplots_by_country.pdf", width=11, height=8)
par(mfrow=c(5,8) , mar=c(2.5,2.5,2.5,1.5))
for (i in seq(1,41)[-2]){
  bdm = as.matrix(rbind( rev(stg_grant[i,]), rev(stg_eval[i,]) - rev(stg_grant[i,]) ))
  b=barplot( bdm, names=rev(sub("X","",names(stg_eval))), ylim = c(0,ifelse(max(bdm)<10,10,max(bdm))), las=2,
             col=c("#007837","#eed4a2ff"), cex.names=0.5, cex.axis=1.4, main=country_names_mod[i] )
}
dev.off()
